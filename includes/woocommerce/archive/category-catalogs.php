...
$term_object = 'term_' . $term->term_id;

$title    = get_field( 'title', $term_object );
$text     = get_field( 'text', $term_object );
$catalogs = get_field( 'category_catalogs', $term_object );
    ...
    <div class="container">
        ...
        <div class="catalogs-grid">
            ...
            <div class="catalogs-image">
                <?php if ( ! empty( $catalogs ) && is_array( $catalogs ) && isset( $catalogs[0]['image__gif'] ) ) :
                    $image = $catalogs[0]['image__gif'];
                    ?>
                    <img
                            id="catalog-preview"
            ...
            <div class="catalogs-content">
                <div class="catalogs_top">
                    <?php if ( $title ) : ?>
                        <h2><?php echo esc_html( $title ); ?></h2>
                    <?php endif; ?>

                    <?php if ( $text ) : ?>
                        <p class="intro"><?php echo esc_html( $text ); ?></p>
                    <?php endif; ?>
                </div>
                <?php if ( $catalogs ) : ?>
                    <ul class="catalogs-list">
                        <?php foreach ( $catalogs as $index => $item ) : ?>
                            <li
                                    class="catalog-item"
                                    data-image="<?php echo esc_url( $item['image__gif']['url'] ?? '' ); ?>"
                            >
                                <a
                                        href="<?php echo esc_url( $item['catalog']['url'] ?? '#' ); ?>"
                                        target="_blank"
                                >
                                    <span class="icon">
                                    ...
                                    <span class="title">
                    <?php echo esc_html( $item['catalog_title'] ); ?>
                  </span>
                                    <span class="arrow">
                                            <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none"
                                                 xmlns="http://www.w3.org/2000/svg">
