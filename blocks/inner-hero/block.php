...
// Δυναμικές κλάσεις
$hero_color_class = ' inner-hero--' . sanitize_title($hero_color);

/**
 * ΛΟΓΙΚΗ ΚΟΥΜΠΙΟΥ (PHP Variant)
 * Αν το mode είναι Light (λευκά γράμματα), θέλουμε το λευκό κουμπί.
 * Αν το mode είναι Dark (μαύρα γράμματα), θέλουμε το μαύρο κουμπί.
 */
$button_variant = (strtolower($hero_color) === 'light') ? 'white-black' : 'black-white';
...
// detect video platform (logic remains same)
$video_type = '';
$video_embed = '';
if ($video_url) {
...
<div class="inner-hero section-full-width<?= esc_attr($hero_color_class); ?> <?= esc_attr($block['className'] ?? ''); ?>"
    ...
    <div class="inner-hero__inner container sec_padding">
        <div class="inner-hero__content">
            ...
            <div class="bottom_content" data-animate="fade-up" data-animate-trigger="load" data-animate-delay="0.45">
                ...
                <?php if (!empty($sub_texts['subtext'])): ?>
                    <p class="inner-hero__text"><?= esc_html($sub_texts['subtext']); ?></p>
                <?php endif; ?>

                <?php
                $button = $sub_texts['button_link'] ?? null;
                if (!empty($button)) {
                    $button_variant = (strtolower($hero_color) === 'light') ? 'white' : 'black';

                    rv_button_arrow([
                            'text'          => $button['title'] ?? 'Επικοινωνήστε μαζί μας',
                            'url'           => $button['url'] ?? '#',
                            'target'        => $button['target'] ?? '_self',
                            'variant'       => $button_variant, // Χρήση σωστού variant για τη συνάρτηση
                            'icon_position' => 'left',
                    ]);
                }
                ?>
            ...
            <div class="inner-hero__boxes" data-animate="stagger-fade" data-animate-trigger="load" data-animate-delay="0.6" data-animate-stagger="0.1">
                <?php foreach ($hero_boxes as $box): ?>
                    <article class="hero-box">
                        ...
                        <?php if ($box['image']): ?>
                            <div class="hero-box__image"><img src="<?= esc_url($box['image']['url']); ?>" alt=""></div>
                        <?php endif; ?>
                        <div class="hero-box__content">
                            ...
                            <div class="hero-box__link">
                                ...
                                <span class="hero-box__icon">
                                    <svg width="7" height="9" viewBox="0 0 7 9"><path d="M0.977505 0.977581L5.12476 4.48757L0.977504 7.99756" stroke="white" stroke-width="1.95503" stroke-linecap="round" fill="transparent" /></svg>
                                </span>
                            </div>
                        </div>
                    </article>
