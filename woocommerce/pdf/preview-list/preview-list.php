...
// The admin upload field actually stores the attachment ID under
// "{option_id}-yith-attachment-id", not "ywraq_pdf_logo-yith-attachment-id"
// (that's a long-standing mismatch in YITH's own stock templates).
$logo_attachment_id = apply_filters( 'yith_pdf_logo_id', get_option( 'ywraq_download_pdf_on_request_logo-yith-attachment-id' ) );
if ( ! $logo_attachment_id && $logo_url ) {
	$logo_attachment_id = attachment_url_to_postid( $logo_url );
}
// get_attached_file() (server filesystem path) on purpose: this template is
// only ever rendered through mPDF server-side (never shown to a browser),
// so mPDF reads the file straight from disk instead of doing a remote HTTP
// fetch of its own site (which is what breaks on local/staging SSL setups).
$logo = $logo_attachment_id ? get_attached_file( $logo_attachment_id ) : $logo_url;

// Use the file extension directly instead of wp_check_filetype(): WordPress
// doesn't recognize .svg as an allowed mime type out of the box, so the old
// jpeg/png-only whitelist silently dropped an SVG logo.
$logo_ext = $logo ? strtolower( pathinfo( $logo, PATHINFO_EXTENSION ) ) : '';
$logo     = apply_filters(
	'ywraq_pdf_logo',
	in_array( $logo_ext, array( 'jpg', 'jpeg', 'png', 'svg' ), true ) ? $logo : ''
);

// SVGs have no intrinsic width/height (only a viewBox), so mPDF renders them
// oversized unless we pass explicit width/height. Read the viewBox to keep
// the logo's own aspect ratio at a fixed target width.
$logo_width  = 160;
$logo_height = round( $logo_width * 0.2 );

if ( $logo && 'svg' === $logo_ext ) {
	$svg = @simplexml_load_file( $logo ); //phpcs:ignore
	if ( $svg && isset( $svg['viewBox'] ) ) {
		$viewbox = preg_split( '/[\s,]+/', trim( (string) $svg['viewBox'] ) );
		if ( 4 === count( $viewbox ) && (float) $viewbox[2] > 0 ) {
			$logo_height = round( $logo_width * ( (float) $viewbox[3] / (float) $viewbox[2] ) );
		}
	}
}
...
<div class="pdf-header">
	<?php if ( $logo ) : ?>
		<img class="logo" src="<?php echo esc_url( apply_filters( 'ywraq_downloar_pdf_logo_src', $logo ) ); ?>" width="<?php echo esc_attr( $logo_width ); ?>" height="<?php echo esc_attr( $logo_height ); ?>" alt="">
	<?php endif; ?>
	<div class="header-contact">
		<div><?php echo esc_html( $shop_address ); ?></div>
		<div><?php echo esc_html( $shop_phone ); ?></div>
	</div>
</div>
				...
				<tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>" <?php echo esc_attr( apply_filters( 'yith_ywraq_item_attributes', '', $raq_content, $key ) ); ?>>
					<?php if ( $show_thumbnail ) : ?>
						<td class="product-thumbnail" width="80px">
							...
							if ( $image_id ) {
								...
								// get_attached_file() on purpose — see note on $logo above.
							...
							$thumbnail = sprintf( '<img src="%s" class="thumbnail-img" alt=""/>', esc_attr( $thumbnail_url ) );
					...
					<?php if ( $show_quantity ) : ?>
						<td class="product-quantity number">
							<?php echo esc_html( $raq['quantity'] ); //@phpcs:ignore ?>
						</td>
					<?php endif; ?>
					<?php if ( $show_single_price ) : ?>
